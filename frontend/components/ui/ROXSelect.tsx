  ...
  return (
    <div className={`w-full space-y-2 ${className}`} ref={containerRef}>
      ...
      <div className="relative">
        <motion.div
          className={`relative overflow-hidden rounded-xl bg-black/50 backdrop-blur border cursor-pointer flex items-center justify-between min-h-[56px] px-4 hover:border-yellow-400/30 transition-all duration-300 ${
            ...
            >
              <div className="p-3">
                {options.map((option, index) => (
                  <motion.div
                    key={option.value}
                    className={`px-4 py-3 cursor-pointer rounded-xl transition-all duration-300 border border-transparent mb-1 ${
                      value === option.value
      ...
      {error && (
        <p className="mt-2 text-sm text-red-400 bg-red-400/10 border border-red-400/20 px-3 py-2 rounded-lg">
          {error}
        </p>
      )}
    </div>
