...
Fixed to match colleges by name instead of UNITID
"""
...
class RealMajorMapping:
    ...
    def find_college_match(self, ipeds_unitid: int) -> Optional[str]:
        """Find matching college name for an IPEDS UNITID"""
        # For now, we'll need to create a mapping manually or use a different approach
        # Let me check if we can get college names from the IPEDS data itself
        
        # Check if there are other sheets with college names
        try:
            # Try to read other sheets
            excel_file = pd.ExcelFile("therealdatabase/ipeds_top_majors_2024.xlsx")
            print(f"Available sheets: {excel_file.sheet_names}")
            
            # Try the 'Top10_Long' sheet which might have college names
            if 'Top10_Long' in excel_file.sheet_names:
                long_df = pd.read_excel("therealdatabase/ipeds_top_majors_2024.xlsx", sheet_name='Top10_Long')
                print(f"Top10_Long sheet shape: {long_df.shape}")
                print(f"Top10_Long columns: {list(long_df.columns)}")
                
                # Look for college name column
                name_cols = [col for col in long_df.columns if any(keyword in col.lower() 
                           for keyword in ['name', 'institution', 'college', 'university'])]
                
                if name_cols:
                    print(f"Potential name columns: {name_cols}")
                    # Create UNITID to name mapping
                    unitid_name_map = {}
                    for _, row in long_df.iterrows():
                        unitid = row.get('UNITID')
                        name = row.get(name_cols[0]) if name_cols else None
                        if pd.notna(unitid) and pd.notna(name):
                            unitid_name_map[int(unitid)] = str(name)
                    
                    print(f"Created UNITID to name mapping for {len(unitid_name_map)} colleges")
                    return unitid_name_map.get(ipeds_unitid)
    ...
    def create_mappings(self):
        ...
        # First, try to get college names from IPEDS data
        unitid_to_name = {}
        try:
            excel_file = pd.ExcelFile("therealdatabase/ipeds_top_majors_2024.xlsx")
            if 'Top10_Long' in excel_file.sheet_names:
                ...
                if name_cols:
                    ...
                    for _, row in long_df.iterrows():
                        ...
                        if pd.notna(unitid) and pd.notna(name):
                            unitid_to_name[int(unitid)] = str(name)
                    
                    print(f"Found {len(unitid_to_name)} colleges with names in IPEDS data")
                else:
                    print("No name column found in Top10_Long sheet")
            else:
                print("Top10_Long sheet not found")
        except Exception as e:
            print(f"Error reading IPEDS name data: {e}")
        ...
        print(f"Created major data for {matched_colleges} colleges")
...
def main():
    ...
    print("Creating Real Major Mapping System (Fixed)")
    print("=" * 50)
